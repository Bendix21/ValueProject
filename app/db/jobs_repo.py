...
async def insert_job(
    ...
    async with await psycopg.AsyncConnection.connect(db_url) as conn:
        await conn.execute(
            "INSERT INTO jobs (job_id, target_url, max_scenarios, max_pages) "
            "VALUES (%s, %s, %s, %s)",
            (job_id, target_url, max_scenarios, max_pages),
        )
        await conn.commit()
...
async def list_jobs(db_url: str) -> list[dict]:
    async with await psycopg.AsyncConnection.connect(db_url) as conn:
        async with conn.cursor(row_factory=dict_row) as cur:
            await cur.execute(
                "SELECT job_id, target_url, max_scenarios, max_pages, created_at "
